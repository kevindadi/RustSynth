...
use super::{JsonPetriNet, PetriNetBuilder};
...
/// ```
pub fn convert_rustdoc_to_petri(
    ...
    // 4. 填充元数据
    json_net.metadata.crate_name = crate_data
        .index
        .get(&crate_data.root)
...
pub fn convert_rustdoc_to_petri_with_options(
    ...
    json_net.metadata.source_file = Some(format!(
        ...
        options.include_private, options.generate_guards
    ));
            ...
            if let Some(first_input) = transition.inputs.first() {
                match first_input.mode.as_deref() {
                    Some("&") => {
                        if !transition
                            .guard_refs
                            .contains(&"g_shared_borrow".to_string())
                        {
...
pub fn batch_convert<P: AsRef<Path>>(
    ...
    for input_file in input_files {
        ...
        output_files.push(output_path.to_str().unwrap_or("unknown").to_string());
    }
