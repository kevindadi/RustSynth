...
pub fn convert_ir_to_lpn(ir: &IrGraph) -> LabeledPetriNet {
    ...
    for node_idx in ir.type_graph.node_indices() {
        ...
        let lpn_idx = if is_place {
            ...
            lpn.add_transition(node_label.clone())
        };
    ...
    for edge_ref in ir.type_graph.edge_references() {
        ...
        if let (Some(&(source_is_place, source_lpn_idx)), Some(&(target_is_place, target_lpn_idx))) =
...
impl LabeledPetriNet {
    ...
    pub fn to_pnml(&self) -> String {
        ...
        xml.push('\n');
        xml.push_str(r#"  <net id="ir_graph_net" type="http://www.pnml.org/version-2009/grammar/ptnet">"#);
        xml.push('\n');
...
"#,
    ...
    pub fn to_dot(&self) -> String {
        ...
        for (idx, trans) in self.transitions.iter().enumerate() {
            dot.push_str(&format!(
                "  t{} [label=\"{}\" shape=box fillcolor=palegreen];\n",
                idx,
                escape_dot(trans)
            ));
        }
    ...
    pub fn save_to_file<P: AsRef<std::path::Path>>(
        ...
        let content = match format {
            ...
            ExportFormat::Json => self.to_json().map_err(|e| {
